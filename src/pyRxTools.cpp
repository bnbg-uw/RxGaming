    ...
    while (true) {
        ...
        try {
            ...
            if (mask.dataOverlapsMultiPolygon(poly)) {
                ...
                for (int j = 0; j < l.nFeature(); j++) {
                    ...
                    if (e) {
                        if (poly.overlapsExtent(e.value())) {
                            ...
                            lapis::VectorDataset<lapis::Point> pts(lidarDataset->highPoints(j)->string());
                            auto polyE = poly.boundingBox();
                            for (int k = 0; k < pts.nFeature(); k++) {
                                auto xy = lidarDataset->coordGetter()(pts.getFeature(k));
                                if (polyE.contains(xy.x, xy.y)) {
                                    if (poly.containsPoint(lapis::Point(xy, thisMhm.crs()))) {
                                        auto val = thisBasinMap.atXY(xy.x, xy.y);
                                        if (!val.has_value()) {
                                            continue;
                                        }
                                        double a = (double)basinIdToNcells[val.value()] * thisBasinMap.xres() * thisBasinMap.yres();
                                        auto height = lidarDataset->heightGetter()(pts.getFeature(k));
                ...
                if (!mhm.size()) {
                    std::cout << "No overlapping tiles found for this Rx. Skipping.\n";
                    continue;
                }
...
void setTaos(int idx, double* taoData, int size) {
    ...
    rxtools::TaoList taos(rxs[idx].unitMask.crs());
    for (size_t i = 0; i < size / 5; ++i) {
        taos.addTao(lapis::CoordXY(taoData[5 * i], taoData[5 * i + 1]), //xy
            taoData[5 * i + 3], //height
            3, //radius
            taoData[5 * i + 2], //area
            dbhModel.predict(taoData[5 * i + 3], lapis::linearUnitPresets::meter, lapis::linearUnitPresets::centimeter) //dbh
        );
    }
    rxs[idx].taos = taos;
}
...
void getRawClumps(int idx, int* ids) {
    ...
    for (size_t i = 0; i < rxs[idx].taos.size(); ++i) {
        g.addTAO(rxs[idx].taos.node(i), lapis::lico::NodeStatus::on);
    }
...
void getSimulatedStructures(int idx, double bbDbh, double* out) {
    try {
        ...
        rxtools::TaoList base(rx.taos.crs());
        for (size_t i = 0; i < rx.taos.size(); i++) {
            if (rx.taos.dbh(i) >= bbDbh) {
                base.addTao(rx.taos.xy(i),
                    rx.taos.height(i),
                    rx.taos.radius(i),
                    rx.taos.area(i),
                    rx.taos.dbh(i));
        ...
        for (int i = 1; i < 11; ++i) {
            ...
            while (testTaos.size() < limit && notBBidx.size()) {
                ...
                notBBidx.pop_back();
                testTaos.addTao(rx.taos.xy(idx),
                    rx.taos.height(idx),
                    rx.taos.radius(idx),
                    rx.taos.area(idx),
                    rx.taos.dbh(idx));
...
void getTreatedRawClumps(int idx, int* ids) {
    lapis::lico::GraphLico g{ lapis::Alignment((lapis::Extent)rxs[idx].unitMask, 1, 1) };
    for (size_t i = 0; i < rxs[idx].treatedTaos.size(); ++i) {
        g.addTAO(rxs[idx].treatedTaos.node(i), lapis::lico::NodeStatus::on);
    }
